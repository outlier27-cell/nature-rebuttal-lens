# ReviewWeaver Final Workflow Upgrade Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Upgrade the current NatureReview-Interact v3 workflow into ReviewWeaver, a manuscript-aware, cross-disciplinary Author Rebuttal Assistant workflow that remains non-training, provenance-first, and not rule-generated.
...
- Create `src/peer_review_skills/agents/reviewweaver_agents.py`
...
- Create `src/peer_review_skills/agents/reviewweaver_workflow.py`
  - Adds `run_reviewweaver_workflow`, file-input wrapper, trace serialization, and config redaction reuse.
...
- Modify `src/peer_review_skills/cli/main.py`
  - Add `run-reviewweaver` CLI.
- Create `examples/reviewweaver/`
...
- Create `docs/REVIEWWEAVER_WORKFLOW_zh.md`.
- Modify `UPGRADE.md` with Phase 10.
- Add tests in `tests/test_reviewweaver_workflow.py`.
...
- [ ] Implement `ManuscriptContext`, `ManuscriptSection`, `load_manuscript_context`, and `build_reviewweaver_unit`.
...
### Task 3: ReviewWeaver Orchestrator Integration

- [ ] Write failing tests for a complete ReviewWeaver trace containing manuscript context, evidence map, case interpretation, 12 agent outputs, and responsible warnings.
...
- [ ] Add `run-reviewweaver` CLI supporting `--review-file`, `--manuscript-file`, `--response-file`, `--output-dir`, `--enable-refinement`, and `--limit-cases`.
- [ ] Add example inputs under `examples/reviewweaver/`.

### Task 5: Docs and Naming

- [ ] Update public docs to foreground ReviewWeaver name, manuscript-aware workflow, and cross-disciplinary value.
- [ ] Add `docs/REVIEWWEAVER_WORKFLOW_zh.md`.
...
- `run-reviewweaver` can run with sample text files using a mockable model client path in tests.
...
- Public docs state the project is ReviewWeaver / NatureReview-Interact derived, manuscript-aware, assistant-only, not final rebuttal generation.
