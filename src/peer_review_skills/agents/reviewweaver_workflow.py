"""ReviewWeaver manuscript-aware workflow entry point."""

import json
from pathlib import Path
from typing import Any

from peer_review_skills.agents.manuscript_context import build_reviewweaver_unit
from peer_review_skills.agents.multi_agent_orchestrator import MultiAgentOrchestrator
from peer_review_skills.agents.reviewweaver_agents import create_reviewweaver_frontend_agents
...
DEFAULT_REVIEWWEAVER_OUTPUT_DIR = Path("data/evaluation/reviewweaver_demo")


def run_reviewweaver_workflow(
    *,
    project_root: Path,
    review_text: str,
    response_text: str = "",
    ...
    """Run one manuscript-aware ReviewWeaver workflow trace."""
    config = config or {}
    if config.get("enable_refinement", False):
        raise ValueError(
            "ReviewWeaver refinement is not yet supported; run without enable_refinement "
    ...
    unit = build_reviewweaver_unit(
        ...
        unit_id=str(config.get("unit_id") or "reviewweaver_user_case"),
    )
    ...
    trace = execute_reviewweaver_trace(
        unit=unit,
        retrieval=retrieval,
    ...
    output_dir = Path(config.get("output_dir") or root / DEFAULT_REVIEWWEAVER_OUTPUT_DIR)
    ...
    summary = {
        "system_name": "ReviewWeaver",
        "workflow_version": "reviewweaver_v1",
        "total_traces": 1,
    ...
    (output_dir / "reviewweaver_summary.json").write_text(
...
def execute_reviewweaver_trace(
    *,
    unit: dict[str, Any],
    ...
    """Execute the 12-agent ReviewWeaver workflow for a prepared unit."""
    config = config or {}
    if config.get("enable_refinement", False):
        raise ValueError(
            "ReviewWeaver refinement is not yet supported; run without enable_refinement."
    ...
    agents = {
        **create_reviewweaver_frontend_agents(model_client),
    ...
    trace = orchestrator.execute_reviewweaver_workflow(unit, retrieval, taxonomies)
    trace["system_name"] = "ReviewWeaver"
    trace["workflow_version"] = "reviewweaver_v1"
